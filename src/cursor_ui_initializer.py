# -*- coding: utf-8 -*-
"""
Hybrid UI Automation Script - Cursor UI 初始化模組
...
class CursorUIInitializer:
    """Cursor UI 初始化器"""
    
    def __init__(self):
        """初始化 UI 初始化器"""
        self.logger = get_logger("CursorUIInitializer")
        self.logger.info("Cursor UI 初始化器啟動")
...
# 創建全域實例
ui_initializer = CursorUIInitializer()

# 便捷函數
def initialize_cursor_ui(wait_time: float = None) -> bool:
    """
    初始化 Cursor UI 的便捷函數
...
def prepare_cursor_for_automation() -> bool:
    """
    為自動化準備 Cursor 的便捷函數
